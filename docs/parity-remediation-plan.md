...
- **Recommended next:** all 10 phases + the follow-ups are done. **#16 is now FULLY closed** — block-on-error + OB7 (`2a96231`), **multi-item HTTP batching** (`efb487e`: ungrouped items batched by ItemType in one call; grouped stay serial), and the **operational state machine** (`1ff5fd9`: per-group Running/Paused/Blocked, permanent-failure blocking, pause/resume/unblock/skip controls + `FC_OUTBOX_ADMIN_PORT` admin API). DB-secret rotation (`25b5cc0`) closed too. **Remaining (explicitly deferred, low-priority):** **O3** `?provider=` (niche, clean error today, untestable without a real IdP); **A4b** full passkey convert-on-read (owner: fresh-only — legacy passkeys non-blocking, `2683bdc`); Phase 5/6 minor nits. No net-new phases, no open correctness gaps.
...
- **Phase 8 / #16 — FULLY DONE.** OB4 block-on-error + OB7 (`2a96231`): GroupDistributor stops a group on a failing item, `Repository.Release`-ing the rest to re-run in order behind it (closes the ordering gap); max-concurrent-groups semaphore (`FC_OUTBOX_MAX_CONCURRENT_GROUPS`, default 10); `BlockOnError` default true. **Multi-item HTTP batching** (`efb487e`): `HTTPDispatcher.SendBatch` posts N same-ItemType items in one call + maps per-item results; the processor batches **ungrouped** items by type (grouped stay serial — batching + strict order are at odds). **Operational state machine** (`1ff5fd9`, port of Rust `message_group_processor.rs` into the DB-claim model): a per-group `GroupStateManager` (Running/Paused/Blocked) on the leader-gated processor; a PERMANENT failure (non-retryable / retry-exhausted) of a grouped item Blocks its group (later items withheld, not advanced past the poison); controls PauseGroup/ResumeGroup/UnblockGroup (clears + re-queues the poison via new `Repository.Requeue`)/SkipGroup (clears, leaves it failed) + GroupStates/BlockedGroups; a localhost admin HTTP API (`GET /outbox/groups[/blocked]`, `POST /outbox/groups/{group}/{pause,resume,unblock,skip}`) served on `FC_OUTBOX_ADMIN_PORT`. The in-memory per-group actor + the operational pause/skip controls are the Rust design, realised in Go's poll model rather than as a long-lived actor.
...
- **Drop-in migration safety (RESOLVED 2026-05-31):** `internal/migrate.bootstrap()` now auto-seeds the goose ledger from the Rust platform's `_schema_migrations` tracker (and falls back to baselining a populated-but-untracked schema via the `tnt_clients` marker), so the first Go boot against an existing Rust DB does NOT re-run the destructive partition migrations 019/022. The manual `tools/baseline-goose-ledger.sql` is now an optional override, not a required pre-step. Proven by `TestDropInSafety` (gated `FC_MIGRATE_PG_TEST=1`, embedded Postgres). Earlier note (now moot): "owner accepted msg_events recreation otherwise."
