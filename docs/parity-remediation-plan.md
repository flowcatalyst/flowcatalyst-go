...
## Progress & Handover (updated 2026-05-29d)
...
- **Recommended next:** Phase 10 (ops/Docker — P1–P5). Then close the open follow-ups (#13/#15/#16).
- **Phase 9 follow-ups (minor, not blocking):** (1) `GET /api/me/applications` is missing from the Go platform (only `/api/me` is mounted), so the `list_my_applications` MCP tool 404s — implement it in `internal/platform/shared/me` (Rust has `/api/me/applications`). (2) No live smoke test yet of `fc-dev start` MCP bootstrap + an MCP client end-to-end against the embedded platform; the build + unit tests pass but a runtime walk-through (stdio + `--http`) is worth doing. (3) The bootstrap is idempotent by client existence — if the creds file is deleted but the DB client remains, it won't regenerate (matches Rust); `fc-dev fresh` recreates both.
- **Phase 8 follow-up (tracked, not blocking):** OB4 multi-item batch dispatch + the stateful per-group block-on-error processor (in-memory per-group queues, attempt tracking, block/skip/unblock state machine, mirroring Rust `message_group_processor.rs`) + OB7 explicit max-concurrent-groups semaphore. Current behaviour is correct (per-group FIFO via GroupDistributor; total concurrency bounded by `MaxInFlight`; per-item results honoured; retries capped) — these are throughput/operability refinements.
